...
export const DispatchCart: React.FC<DispatchCartProps> = ({
  ...
  const handleAutoGenerateDocNo = () => {
    ...
    let maxSeq = 0;
    dispatchLots.forEach((l) => {
      const doc = l.transportDocNo || l.lotNumber || '';
      if (doc.includes('DCVRL')) {
    ...
    availableStoragePacks.forEach((p) => {
      if (p.dispatchDocNo && p.dispatchDocNo.includes('DCVRL')) {
        const match = p.dispatchDocNo.match(/(\d+)$/);
        if (match) {
          const num = parseInt(match[1], 10);
          if (!isNaN(num) && num > maxSeq) maxSeq = num;
        }
    ...
    setTransportDocNo(`DCVRL/${fiscal}-${nextSeqStr}`);
  };
                  ...
                  <table className="w-full text-left border-collapse text-xs">
                    ...
                    <tbody className="divide-y divide-slate-100">
                      {parsedDirectPacks.map((dp, idx) => {
                        ...
                        const alreadyDispatched = availableStoragePacks.find((p) => p.packNumber === dp.packNumber && p.status === 'DISPATCHED');
                        ...
                        return (
                          <tr key={dp.id} className="hover:bg-slate-50">
                            ...
                            <td className="p-2.5 font-mono-code font-bold text-slate-900">
                              ...
                              {alreadyDispatched && (
                                <div className="mt-1 px-1.5 py-0.5 rounded bg-rose-50 border border-rose-200 text-rose-700 text-[9px] font-sans font-bold flex items-center gap-1">
                                  <AlertCircle className="w-3 h-3 text-rose-600 flex-shrink-0" />
                                  <span>Already dispatched ({alreadyDispatched.dispatchToCustomer || 'Customer'})</span>
                                </div>
                              )}
                            </td>
                            <td className="p-2.5">
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-1">
                      <label className="font-bold text-slate-700">Document Number</label>
                      <button
                        type="button"
