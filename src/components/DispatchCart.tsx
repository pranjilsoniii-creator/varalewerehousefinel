...
import { BatteryPack, DispatchLot, SavedAddress } from '../types';
import { BATTERY_MODELS, COMMON_TRANSPORTERS, DEFAULT_PLANT_LOCATION, DEFAULT_SAVED_ADDRESSES } from '../data/batteryCatalog';
...
export const DispatchCart: React.FC<DispatchCartProps> = ({
  ...
  // Selected packs for current dispatch lot
  ...
  const handleAuthorizeDispatch = () => {
    ...
    // If user asked to save this new address to directory
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6 space-y-6 animate-fadeIn">
        ...
        <div>
          <div className="flex items-center gap-2 mb-1">
            <span className="px-2.5 py-0.5 rounded-full bg-blue-100 text-blue-700 border border-blue-200 text-xs font-bold flex items-center gap-1.5 uppercase tracking-wider">
              <Truck className="w-3.5 h-3.5 text-blue-600" /> Outward Dispatch Staging
            </span>
          ...
          <h2 className="text-xl sm:text-2xl font-bold text-slate-900 tracking-tight font-display">
            Outward Dispatch Staging & Gate Pass System
          </h2>
          <p className="text-xs sm:text-sm text-slate-500 mt-0.5">
            Stage lithium battery packs from Inward Area, complete 100+ word TO address, and generate official Tata Gate Pass.
          </p>
        </div>

        <div className="flex items-center gap-3">
          <span className="text-xs font-bold text-slate-600">
            Staged in Cart: <span className="font-mono-code text-blue-700 text-sm font-extrabold">{stagedPacks.length}</span> Packs
          </span>
        </div>
      </div>
        ...
        <div className="p-4 rounded-xl bg-emerald-50 border border-emerald-200 flex flex-wrap items-center justify-between gap-3 animate-fadeIn text-xs">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="px-2.5 py-1.5 text-emerald-800 hover:text-emerald-950 font-bold"
            >
              Dismiss
            </button>
          </div>
      ...
      {/* Main Grid: Left Staged Cart + Right Outward Consignee Form */}
            ...
            {stagedPacks.length === 0 ? (
              <div className="p-8 text-center text-slate-500 border border-dashed border-slate-200 rounded-xl space-y-2">
                ...
                <p className="text-[11px] text-slate-400">
                  Add packs from available plant stock below or from Inward Register using the 🚀 button.
                </p>
              </div>
            ) : (
                  ...
                  <thead>
                    <tr className="bg-slate-50 border-b border-slate-200 text-[10px] font-bold text-slate-500 uppercase tracking-wider">
                      <th className="p-3 w-10">
                        <input
                          type="checkbox"
                          checked={selectedPackIds.size === stagedPacks.length}
                          onChange={(e) => {
                            if (e.target.checked) {
                              setSelectedPackIds(new Set(stagedPacks.map((p) => p.id)));
                            } else {
            ...
            <div className="flex items-center gap-2">
              <div className="relative flex-1">
                ...
                <input
                  ...
                  placeholder="Search pack serial number to add..."
                  className="w-full bg-slate-50 border border-slate-200 rounded-lg pl-9 pr-3 py-2 text-xs font-mono-code text-slate-900 focus:bg-white focus:border-blue-500 focus:outline-none"
                />
              </div>
              ...
              {stockPickerSelectedIds.size > 0 && (
                <button
                  ...
                  className="px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg text-xs font-bold transition flex items-center gap-1 cursor-pointer shadow-xs"
                >
                  <Plus className="w-3.5 h-3.5" />
                  <span>Stage ({stockPickerSelectedIds.size})</span>
                </button>
              )}
            </div>
            ...
            <div className="max-h-56 overflow-y-auto border border-slate-200 rounded-xl divide-y divide-slate-100 text-xs">
              {filteredAvailableStock.length === 0 ? (
                <div className="p-4 text-center text-slate-400 text-xs">
                  No packs available matching search in plant storage.
                </div>
              ) : (
